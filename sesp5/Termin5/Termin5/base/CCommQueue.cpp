...
CCommQueue::CCommQueue(Int32 queueSize, CBinarySemaphore& sem) :
		mSignalSem(sem), mSize(queueSize), mCurrentSize(0), mHeadIndex(0), mTailIndex(
				0), mAccessSem(true, true), // free and shared
...
Int32 CCommQueue::getNumOfBytesNeeded(void) {
	return CCommQueue::getNumOfBytesNeeded(mSize);
}

// get the size of a queue with given message
Int32 CCommQueue::getNumOfBytesNeeded(Int32 numOfMsg) {
